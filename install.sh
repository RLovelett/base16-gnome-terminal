...
set_profile_colors() {
  ...
  if [ "$newGnome" = "1" ]
    ...
    local scheme_path=$dir/colors/$scheme
    local palette=$scheme_path/palette
    local bold_color=$scheme_path/bold_color
    local background_color=$scheme_path/background_color
    local foreground_color=$scheme_path/foreground_color
    local use_theme_colors=$scheme_path/use_theme_colors
    local use_theme_background=$scheme_path/use_theme_background
    local bold_color_same_as_fg=$scheme_path/bold_color_same_as_fg

    # set color palette
    dconf write $profile_path/palette "[$(cat $palette)]"

    # set foreground, background and highlight color
    dconf write $profile_path/bold-color "'$(cat $bold_color)'"
    dconf write $profile_path/background-color "'$(cat $background_color)'"
    dconf write $profile_path/foreground-color "'$(cat $foreground_color)'"

    # make sure the profile is set to not use theme colors
    dconf write $profile_path/use-theme-colors "$(cat $use_theme_colors)"
    dconf write $profile_path/use-theme-background "$(cat $use_theme_background)"

    # set highlighted color to be different from foreground color
    dconf write $profile_path/bold-color-same-as-fg "$(cat $bold_color_same_as_fg)"
